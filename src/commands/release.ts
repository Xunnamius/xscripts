                ...
                for (const script of npmScripts) {
                  ...
                  if (script in cwdPackageJsonScripts) {
                    ...
                    if (dryRun && type === 'post') {
                      ...
                      const scriptExitCode = await attemptToRun('npm', ['run', script], {
                        scriptName: script,
                        logger: taskLogger,
...
async function attemptToRun(
  cmd: Parameters<typeof runNoRejectOnBadExit>[0],
  cmdArgs: Parameters<typeof runNoRejectOnBadExit>[1],
  runConfig: Parameters<typeof runNoRejectOnBadExit>[2] & {
    scriptName: string;
    logger: ExtendedLogger;
  }
) {
  const { all: output, exitCode } = await runNoRejectOnBadExit(cmd, cmdArgs, {
    ...runConfig,
    all: true,
    env: {
      ...runConfig.env,
