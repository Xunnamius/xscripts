...
export default function command(
   ...
   * generated.
   */
  const allSchedulerTasks = `${firstSkippablePrereleaseTaskId}-${lastSchedulerPurviewTaskId}`;
            ...
            skipTargets.flatMap((target) => {
              switch (target) {
                case allSchedulerTasks: {
                  return Array.from({
                    length: lastSchedulerPurviewTaskId - firstSkippablePrereleaseTaskId
                  }).map((_, index) => String(firstSkippablePrereleaseTaskId + index));
                }
...
There is also \`--skip-tasks=${allSchedulerTasks}\`, which will skip running tasks ${firstSkippablePrereleaseTaskId} through ${lastSchedulerPurviewTaskId}. This is useful when xscripts is being managed by a task scheduling tool like Turbo that decides out-of-band if/when/how it wants to run these specific tasks; such a tool only calls \`xscripts release\` afterwards, when it's ready to trigger xrelease. Therefore, \`--skip-tasks=${allSchedulerTasks}\` becomes the default when Turbo is detected in the runtime environment (by checking for the existence of \`process.env.TURBO_HASH\`).

If the package's package.json file is missing relevant NPM scripts, this command will exit with an error unless \`--skip-missing-tasks\` is provided, in which case any missing scripts (except "release", which must be defined) are noted in a warning but otherwise ignored.
